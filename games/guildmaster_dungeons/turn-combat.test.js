...
{const nodes=api.classSkillNodes('Paladin'),byId=Object.fromEntries(nodes.map(node=>[node.id,node])),links=id=>byId[id].requiresAny||[byId[id].requires];for(const path of ['str','dex','int'])assert.equal(byId[`${path}-ring-3`].requires,`${path}-ring-0`,`${path.toUpperCase()} ring retains its original connecting edge`);assert.deepEqual(Array.from(links('dex-ring-4')).sort(),['dex-ring-3','str-ring-0'].sort(),'STR-to-DEX uses the marked existing ring nodes');assert.deepEqual(Array.from(links('int-ring-4')).sort(),['dex-ring-3','int-ring-3'].sort(),'DEX-to-INT uses the marked existing ring nodes');assert.deepEqual(Array.from(links('int-ring-0')).sort(),['attribute-int-6','str-ring-3'].sort(),'STR-to-INT uses the marked existing ring nodes')}
