...
for(const className of Object.keys(api.abilities)){const nodes=api.classSkillNodes(className);assert.equal(nodes.length,87,`${className} has a large universal, ring, and class-specific skill network`);assert.equal(nodes.filter(node=>node.basic).length,72,`${className} can enter seventy-two basic progression nodes`);assert.equal(nodes.every(node=>node.maxRank===1),true,`${className} skill nodes each cost exactly one point`);assert.equal(nodes.filter(node=>node.major).length,7,`${className} has four class keystones and three attribute-ring notables`)}
