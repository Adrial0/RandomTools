...
for(const className of Object.keys(api.abilities)){const nodes=api.classSkillNodes(className);assert.equal(nodes.length,39,`${className} has a large universal and class-specific skill network`);assert.equal(nodes.filter(node=>node.basic).length,27,`${className} can enter twenty-seven universal attribute nodes`);assert.equal(nodes.filter(node=>node.major).length,4,`${className} has four transformative major nodes`)}
const skilledMage=api.makeHero(10,'Mage');assert.equal(api.availableSkillPoints(skilledMage),9,'a level-ten character has earned nine skill points');skilledMage.skillRanks={'mage-0-0':3};assert.equal(api.availableSkillPoints(skilledMage),6,'purchased ranks consume earned skill points');assert.equal(api.skillBonus(skilledMage,'int'),12,'ranked nodes apply their full mechanical bonus');
