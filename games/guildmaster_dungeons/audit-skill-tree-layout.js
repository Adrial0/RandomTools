...
const outwardDegree=links.reduce((counts,link)=>(counts[link.from.id]=(counts[link.from.id]||0)+1,counts),{});
const overloaded=Object.entries(outwardDegree).filter(([,count])=>count>3).sort((a,b)=>b[1]-a[1]);
...
console.log(JSON.stringify({bounds:{left:Math.min(...xs),right:Math.max(...xs),top:Math.min(...ys),bottom:Math.max(...ys)},outside:outside.map(node=>node.layoutId||node.id),overloaded,nodeHits,lineHits},null,2));
if(process.argv.includes('--positions'))console.log(nodes.map(({id,layoutId,x,y,requires})=>({id,layoutId,x:Math.round(x),y:Math.round(y),requires})).filter(node=>/blood|affliction|tempo|precision|aether|mana|multiattack|fractured-rhythm|evasive-flurry|resolute|ring-gateway|ring-[0-6]|attribute-(str|dex|int)-(5|6|9|10|11|14|15|16|17)|class-3/.test(`${node.id} ${node.layoutId}`)));
if(process.argv.includes('--degrees'))console.log(overloaded.map(([id,count])=>({id,count,children:links.filter(link=>link.from.id===id).map(link=>link.to.id)})));
if(outside.length||overloaded.length||nodeHits.length||lineHits.length)process.exitCode=1;
