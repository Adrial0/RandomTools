...
type Unit=Hero&{side:'attack'|'defense';hp:number;nextAttack:number;nextActive:number;cast:null|{ends:number;kind:string};statuses:Record<string,{power:number;stacks:number;nextTick:number;expires:number;source:Unit}>};
...
export function resolveArenaBattle(attacker:any,defender:any,seed:number){
  const random=rng(seed),replay:string[]=[],metrics=new Map<Unit,Record<string,any>>();
  const build=(h:Hero,side:'attack'|'defense'):Unit=>({...h,side,hp:h.maxHp,nextAttack:Math.max(250,h.attackInterval||2500),nextActive:2500+Math.floor(random()*1500),cast:null,statuses:{}});
  const attack:Unit[]=attacker.members.map((h:Hero)=>build(h,'attack'));
  const defense:Unit[]=defender.members.map((h:Hero)=>build(h,'defense'));
  ...
  const deal=(source:Unit,target:Unit,mult=1,type=source.damageType||'physical')=>{
  ...
  const cleanse=(source:Unit,target:Unit)=>{const keys=Object.keys(target.statuses);if(!keys.length)return;const count=source.subclass==='lifepriest'?keys.length:1;keys.slice(0,count).forEach(k=>delete target.statuses[k]);metrics.get(source)!.cleanses+=Math.min(count,keys.length);log(`${source.name} cleanses ${target.name}.`)};
  const heal=(source:Unit)=>{const target=[...allies(source)].sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(!target)return;const amount=Math.min(target.maxHp-target.hp,Math.max(2,Math.round(((source.int||1)*.55+(source.weaponPower||8)*.2)*(source.healMult||1))));if(amount>0){target.hp+=amount;metrics.get(source)!.healing+=amount;log(`${source.name} heals ${target.name} for ${amount}.`)}if(cleansers.has(source.subclass))cleanse(source,target)};
  const useActive=(source:Unit)=>{
    ...
    if(casting&&interrupters.has(source.subclass)){casting.cast=null;casting.nextActive=clock+3000;metrics.get(source)!.interrupts++;log(`${source.name} interrupts ${casting.name}!`)}
    const damage=deal(source,target,source.subclass==='assassin'?2.1:source.subclass==='berserker'?1.9:1.55,source.element||source.damageType);
  ...
  while(living(attack).length&&living(defense).length&&clock<120000){
    ...
    for(const unit of [...living(attack),...living(defense)]){
      for(const [key,status] of Object.entries(unit.statuses)){
        if(clock>=status.nextTick&&clock<=status.expires){const damage=Math.min(unit.hp,status.power*status.stacks);unit.hp-=damage;metrics.get(status.source)!.statusDamage+=damage;metrics.get(unit)!.damageTaken+=damage;status.nextTick+=2000}
      ...
      if(clock>=unit.nextActive){unit.cast={ends:clock+700,kind:'active'};log(`${unit.name} begins using ${unit.activeType||'an ability'}.`);continue}
  ...
  log(attackerWon?`${attacker.guildName} wins the Arena battle.`:`${defender.guildName} defends successfully.`);
  return{attackerWon,durationMs:clock,replay,report:[...metrics.values()]};
}
