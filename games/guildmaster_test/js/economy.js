...
function renderRunecrafting(){
  ...
  $('runeInventory').innerHTML=owned.length?owned.sort((a,b)=>runeTier(a[0])-runeTier(b[0])).map(([id,v])=>`<div class="mat">${runeIcon(id)} <span class="resourceTier">Tier ${tierLabel(runeTier(id))}</span> ${RUNES[id]?.name||id} <b>${v}</b></div>`).join(''):'<div class="muted">No runes crafted yet.</div>';
    ...
    ${open?`<div class="recipeLineDetails recipeDetailLayout">
      ...
      <aside class="recipeCraftingBox">
        ...
        <div class="recipeDetailMaterials">
          ${Object.entries(r[3]).map(([k,v])=>{const have=s.materials[k]||0,ok=have>=v;return `<span class="${ok?'enough':'missing'}"><b>${have}/${v}</b> ${RESOURCE_NAMES[k]||k} <small>Tier ${tierLabel(resourceTier(k))}</small>${BOSS_RESOURCES.has(k)&&!s.discoveredResources.includes(k)?`<em>${BOSS_RESOURCE_SOURCE[k]}</em>`:''}</span>`}).join('')}
        </div>
