...
function receiveInventoryItem(it,source='loot'){if(!it)return false;s.inventory.push(it);return true}
function selectedInventoryForAction(mode){return s.inventory.filter(it=>selectedInventoryItems.has(it.id)&&!it.starter&&(mode!=='scrap'||!!recipeForItem(it)))}
function performBulkInventoryAction(mode){const items=selectedInventoryForAction(mode);if(!items.length)return notify(`No selected items can be ${mode==='sell'?'sold':'scrapped'}.`);if(mode==='sell'){let gold=0;items.forEach(it=>{gold+=itemSellValue(it);detachItem(it)});const ids=new Set(items.map(it=>it.id));s.inventory=s.inventory.filter(it=>!ids.has(it.id));s.gold+=gold;ids.forEach(id=>selectedInventoryItems.delete(id));save();closeModal();render();notify(`Sold ${items.length} selected item${items.length===1?'':'s'} for ${gold} gold.`,'good');return}let scrapped=0;const removeIds=new Set();items.forEach(it=>{const recovered=scrapItemCore(it);if(recovered){detachItem(it);removeIds.add(it.id);scrapped++}});s.inventory=s.inventory.filter(it=>!removeIds.has(it.id));removeIds.forEach(id=>selectedInventoryItems.delete(id));save();closeModal();render();notify(`Scrapped ${scrapped} selected item${scrapped===1?'':'s'}.`,'good')}
function confirmBulkInventoryAction(mode){const items=selectedInventoryForAction(mode);if(!items.length)return notify(`No selected items can be ${mode==='sell'?'sold':'scrapped'}.`);const value=mode==='sell'?items.reduce((sum,it)=>sum+itemSellValue(it),0):0;showModal(`${mode==='sell'?'Sell':'Scrap'} selected items`,`<div class="card bulkActionWarning"><div class="name">This will permanently ${mode} ${items.length} selected item${items.length===1?'':'s'}.</div><div class="muted">${mode==='sell'?`You will receive ${value.toLocaleString()} gold.`:'Crafting materials will be recovered where storage permits. Equipped items will be unequipped.'}</div><div class="modalActionRow"><button class="btn ${mode==='sell'?'gold':'danger'}" onclick="performBulkInventoryAction('${mode}')">Confirm ${mode} ${items.length} item${items.length===1?'':'s'}</button></div></div>`)}
function bulkSellSelected(){confirmBulkInventoryAction('sell')}
function bulkScrapSelected(){confirmBulkInventoryAction('scrap')}
function inventoryCleanupItems(){return s.inventory.filter(it=>!it.equipped&&!it.starter&&rarityAtOrBelow(it.rarity,inventoryCleanupRarity)&&(inventoryCleanupMode!=='scrap'||!!recipeForItem(it)))}
function confirmInventoryCleanup(){const items=inventoryCleanupItems();if(!items.length)return notify(`No unequipped items up to ${inventoryCleanupRarity} can be ${inventoryCleanupMode==='sell'?'sold':'scrapped'}.`);const value=inventoryCleanupMode==='sell'?items.reduce((sum,it)=>sum+itemSellValue(it),0):0;closeInventoryMenus();showModal(`${inventoryCleanupMode==='sell'?'Sell':'Scrap'} inventory items`,`<div class="card bulkActionWarning"><div class="name">This will permanently ${inventoryCleanupMode} exactly ${items.length} item${items.length===1?'':'s'}.</div><div class="muted">${inventoryCleanupMode==='sell'?`You will receive ${value.toLocaleString()} gold.`:'Materials will be recovered where storage permits. All listed items will still be destroyed if storage fills.'} Equipped and starter items are protected. Future loot is unaffected.</div><div class="modalActionRow"><button class="btn ${inventoryCleanupMode==='sell'?'gold':'danger'}" onclick="performInventoryCleanup()">Confirm ${inventoryCleanupMode} ${items.length} item${items.length===1?'':'s'}</button></div></div>`)}
function performInventoryCleanup(){const items=inventoryCleanupItems();if(!items.length){closeModal();return notify('There are no longer any matching items.')}const ids=new Set(items.map(it=>it.id));let gold=0;items.forEach(it=>{if(inventoryCleanupMode==='sell')gold+=itemSellValue(it);else scrapItemCore(it)});s.inventory=s.inventory.filter(it=>!ids.has(it.id));ids.forEach(id=>selectedInventoryItems.delete(id));if(inventoryCleanupMode==='sell')s.gold+=gold;save();closeModal();render();notify(inventoryCleanupMode==='sell'?`Sold ${items.length} item${items.length===1?'':'s'} for ${gold.toLocaleString()} gold.`:`Scrapped ${items.length} item${items.length===1?'':'s'}.`,'good')}
...
let inventoryCleanupMode='scrap';
let inventoryCleanupRarity='Common';
...
function setInventoryCleanupMode(mode){inventoryCleanupMode=['sell','scrap'].includes(mode)?mode:'scrap';updateInventoryControlUI()}
function setInventoryCleanupRarity(rarity){if(rar.includes(rarity))inventoryCleanupRarity=rarity;updateInventoryControlUI()}
function updateInventoryControlUI(){const rb=$('rarityFilterButton');if(rb)rb.textContent=`Rarity ${inventoryRarityFilter.size}/${rar.length} ▾`;document.querySelectorAll('[data-inv-rarity]').forEach(b=>b.classList.toggle('on',inventoryRarityFilter.has(b.dataset.invRarity)));if($('inventoryCleanupMode'))$('inventoryCleanupMode').value=inventoryCleanupMode;if($('inventoryCleanupRarity'))$('inventoryCleanupRarity').value=inventoryCleanupRarity;if($('inventoryCleanupButton'))$('inventoryCleanupButton').textContent=`Bulk ${inventoryCleanupMode==='sell'?'Sell':'Scrap'} ≤ ${inventoryCleanupRarity} ▾`;if($('inventoryCleanupApply')){const count=inventoryCleanupItems().length;$('inventoryCleanupApply').textContent=`${inventoryCleanupMode==='sell'?'Sell':'Scrap'} ${count} Item${count===1?'':'s'}`;$('inventoryCleanupApply').disabled=!count;$('inventoryCleanupApply').classList.toggle('gold',inventoryCleanupMode==='sell');$('inventoryCleanupApply').classList.toggle('danger',inventoryCleanupMode==='scrap')}if($('inventorySelectToggle'))$('inventorySelectToggle').textContent=inventorySelectionMode?'Done Selecting':'Select Items';if($('inventoryBulkBar'))$('inventoryBulkBar').style.display=inventorySelectionMode?'flex':'none';if($('inventorySelectedCount'))$('inventorySelectedCount').textContent=selectedInventoryItems.size+' selected'}
...
function inventorySortValue(it,key){
  ...
  if(key==='attackSpeed')return it.slot==='Weapon'?1/Math.max(.01,weaponAttackTime(it.weaponTemplate||it.weaponType||it.name)):0;
  const gearMult=it.slot==='Weapon'&&weaponHands(it)===2?2:1;
  return (Number(z[key])||0)*gearMult;
}
...
function inventorySummaryHtml(it){
  ...
  return stats.length?`<div class="inventorySummaryGrid stats-${stats.length} ${it.slot==='Weapon'?'weaponStats':''}">${stats.map(([label,value])=>`<div class="inventorySummaryStat"><span>${label}</span><strong>${value}</strong></div>`).join('')}</div>`:'';
}
function renderInv(){
