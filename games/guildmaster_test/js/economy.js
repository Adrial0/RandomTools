...
function selectedInventoryForAction(mode,useAutoRule=false){const rule=s.inventoryAuto||{rarity:'Common'};return s.inventory.filter(it=>selectedInventoryItems.has(it.id)&&!it.starter&&(!useAutoRule||rarityAtOrBelow(it.rarity,rule.rarity))&&(mode!=='scrap'||!!recipeForItem(it)))}
function performBulkInventoryAction(mode,useAutoRule=false){const items=selectedInventoryForAction(mode,useAutoRule);if(!items.length)return notify(`No selected items can be ${mode==='sell'?'sold':'scrapped'}.`);if(mode==='sell'){let gold=0;items.forEach(it=>{gold+=itemSellValue(it);detachItem(it)});const ids=new Set(items.map(it=>it.id));s.inventory=s.inventory.filter(it=>!ids.has(it.id));s.gold+=gold;ids.forEach(id=>selectedInventoryItems.delete(id));save();closeModal();render();notify(`Sold ${items.length} selected item${items.length===1?'':'s'} for ${gold} gold.`,'good');return}let scrapped=0;const removeIds=new Set();items.forEach(it=>{const recovered=scrapItemCore(it);if(recovered){detachItem(it);removeIds.add(it.id);scrapped++}});s.inventory=s.inventory.filter(it=>!removeIds.has(it.id));removeIds.forEach(id=>selectedInventoryItems.delete(id));save();closeModal();render();notify(`Scrapped ${scrapped} selected item${scrapped===1?'':'s'}.`,'good')}
function confirmBulkInventoryAction(mode,useAutoRule=false){const items=selectedInventoryForAction(mode,useAutoRule);if(!items.length)return notify(`No selected items can be ${mode==='sell'?'sold':'scrapped'}.`);const value=mode==='sell'?items.reduce((sum,it)=>sum+itemSellValue(it),0):0;showModal(`${mode==='sell'?'Sell':'Scrap'} selected items`,`<div class="card bulkActionWarning"><div class="name">This will permanently ${mode} ${items.length} selected item${items.length===1?'':'s'}.</div><div class="muted">${mode==='sell'?`You will receive ${value.toLocaleString()} gold.`:'Crafting materials will be recovered where storage permits. Equipped items will be unequipped.'}</div><div class="modalActionRow"><button class="btn ${mode==='sell'?'gold':'danger'}" onclick="performBulkInventoryAction('${mode}',${useAutoRule})">Confirm ${mode} ${items.length} item${items.length===1?'':'s'}</button></div></div>`)}
function bulkSellSelected(){confirmBulkInventoryAction('sell')}
function bulkScrapSelected(){confirmBulkInventoryAction('scrap')}
function applyInventoryAutoToSelected(){const mode=s.inventoryAuto?.mode;if(!['sell','scrap'].includes(mode))return notify('Choose Auto-sell or Auto-scrap first.');confirmBulkInventoryAction(mode,true)}
...
function updateInventoryControlUI(){const rb=$('rarityFilterButton');if(rb)rb.textContent=`Rarity ${inventoryRarityFilter.size}/${rar.length} ▾`;document.querySelectorAll('[data-inv-rarity]').forEach(b=>b.classList.toggle('on',inventoryRarityFilter.has(b.dataset.invRarity)));s.inventoryAuto=Object.assign({mode:'off',rarity:'Common'},s.inventoryAuto||{});if($('inventoryAutoMode'))$('inventoryAutoMode').value=s.inventoryAuto.mode;if($('inventoryAutoRarity'))$('inventoryAutoRarity').value=s.inventoryAuto.rarity;if($('inventoryAutoButton')){const label=s.inventoryAuto.mode==='sell'?'Auto-sell':s.inventoryAuto.mode==='scrap'?'Auto-scrap':'Auto: Off';$('inventoryAutoButton').textContent=s.inventoryAuto.mode==='off'?label+' ▾':`${label} ≤ ${s.inventoryAuto.rarity} ▾`}if($('inventoryAutoApply')){const mode=s.inventoryAuto.mode,count=['sell','scrap'].includes(mode)?selectedInventoryForAction(mode,true).length:0;$('inventoryAutoApply').textContent=mode==='off'?'Select an action':`${mode==='sell'?'Sell':'Scrap'} ${count} selected item${count===1?'':'s'}`;$('inventoryAutoApply').disabled=!count}if($('inventorySelectToggle'))$('inventorySelectToggle').textContent=inventorySelectionMode?'Done Selecting':'Select Items';if($('inventoryBulkBar'))$('inventoryBulkBar').style.display=inventorySelectionMode?'flex':'none';if($('inventorySelectedCount'))$('inventorySelectedCount').textContent=selectedInventoryItems.size+' selected'}
