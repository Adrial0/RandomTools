...
function queueProfessionJob(pid,kind,key,qty=1){
  ...
  if(worker.busy&&worker.professionBusy!==pid)return notify(`${worker.name} is currently away.`);
...
function setActiveProfession(pid){if(!PROFESSION_DEFS[pid])return;activeProfession=pid;renderCraft()}
...
function renderProfessionRecipe(r,index){
  const pid=recipeProfession(r),p=professionState(pid),req=professionRecipeRequirement(r),max=maxCraftQuantity(r),known=professionRecipeKnown(r),preview=recipePreview(r);
  if(!known)return'';const profile=preview?.profile||[r[1]],stats=preview?.stats||[];
  return `<div class="recipeLine ${p.level<req?'recipeLocked':''}"><div class="recipeLineHeader"><div class="recipeLineIcon">${r[1]==='Material'?gameIcon('resource',r[5]?.outputResource,'◇'):r[1]==='Weapon'?(WEAPONS[r[2]]?.icon||'⚔️'):(itemIcons[r[1]]||'◇')}</div><div><div class="recipeLineName">${r[0]}</div><div class="recipeLineType">${tierLabel(r[4])} · ${r[1]}</div></div></div><div class="recipeLineDetails recipeDetailLayout"><div class="recipeItemInformation"><section class="recipeInfoSection"><h4>Item Profile</h4><div class="recipeProfileList">${profile.map(x=>`<span>${x}</span>`).join('')}</div></section><section class="recipeInfoSection"><h4>Result</h4><div class="recipeStatList">${stats.map(x=>`<span>${x}</span>`).join('')||'<span>Processed crafting material</span>'}</div></section></div><aside class="recipeCraftingBox"><h4>Requirements</h4><div class="recipeCraftMeta"><span>${PROFESSION_DEFS[pid].name} ${req}</span><span>+${professionRecipeXp(r)} XP</span><span>${Math.ceil(jobDuration(pid,'recipe',index,professionEffects(pid))/1000)}s</span></div><div class="recipeCostLabel">Material cost</div><div class="recipeDetailMaterials">${professionMaterialRows(r[3])}</div><div class="recipeLineAction">${p.level>=req&&max>0?`<div class="recipeQuantityControl"><span>×</span><input id="profQty-${index}" type="number" min="1" max="${max}" value="1"><button class="btn gold" onclick="craft(${index},$('profQty-${index}').value)">Create</button></div>`:`<button class="btn" disabled>${p.level<req?'Requires '+PROFESSION_DEFS[pid].name+' '+req:'Missing materials'}</button>`}</div></aside></div></div>`;
}
function renderMealProfession(){return Object.entries(MEALS).sort((a,b)=>(a[1].tier||1)-(b[1].tier||1)).map(([mid,m])=>`<div class="card professionSimpleRecipe"><div><div class="name">${m.icon||'🍲'} ${m.name}</div><div class="muted">${tierLabel(m.tier||1)} · Cooking ${m.level} · +${m.xp} XP · ${m.desc}</div></div><div class="recipeDetailMaterials">${professionMaterialRows(m.cost)}</div><div class="recipeQuantityControl"><span>×</span><input id="mealQty-${mid}" type="number" min="1" value="1"><button class="btn gold" onclick="cookMeal('${mid}',$('mealQty-${mid}').value)">Cook</button></div></div>`).join('')}
function renderRuneProfession(){
  ...
  return `<div class="card"><div class="name">Rune Vault</div><div class="muted">${owned}</div><div class="professionSocketList">${socketable||'<span class="muted">No unequipped socketable equipment.</span>'}</div></div>`+Object.entries(RUNES).sort((a,b)=>runeTier(a[0])-runeTier(b[0])).map(([rid,r])=>`<div class="card professionSimpleRecipe"><div><div class="name">${runeIcon(rid)} ${r.name}</div><div class="muted">${tierLabel(runeTier(rid))} · ${r.desc}</div></div><div class="recipeDetailMaterials">${professionMaterialRows(r.cost)}</div><button class="btn gold" onclick="craftRune('${rid}',1)">Create Rune</button></div>`).join('')}
...
function renderCraft(){
  ...
  $('professionWorkstation').innerHTML=`<div class="professionStation"><div class="professionStationTop"><div><div class="sectionKicker">Workstation</div><h3>${def.icon} ${def.name} <span class="chip">Lv. ${p.level}</span></h3><div class="muted">${def.desc}</div></div><button class="btn ${worker?'':'gold'}" onclick="openProfessionWorkerPicker('${activeProfession}')">${worker?'Change specialist':'Assign specialist'}</button></div><div class="progressMeta"><span>${p.xp.toLocaleString()} / ${need.toLocaleString()} XP</span><span>${worker?worker.name:'Vacant'}</span></div><div class="progressTrack"><div class="progressFill" style="width:${clamp(p.xp/need*100,0,100)}%"></div></div><div class="professionEffectRow">${worker?`<span class="chip">${professionTraitText(worker)}</span><span class="chip ${Object.values(effects).some(Boolean)?'good':''}">${effectSummary(effects)}</span>${p.jobs.length?'<span class="chip">Busy until queue is empty</span>':'<span class="chip">Available until work begins</span>'}`:'<span class="muted">A permanent specialist must be assigned before work can begin.</span>'}</div></div>`;
  ...
  $('recipeList').innerHTML=activeProfession==='cooking'?renderMealProfession():activeProfession==='runecrafting'?renderRuneProfession():recipes.map((r,i)=>recipeProfession(r)===activeProfession?renderProfessionRecipe(r,i):'').join('')||'<div class="empty">No recipes discovered for this profession.</div>';
