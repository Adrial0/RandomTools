...
function professionUnlockedTier(){return Math.max(1,Math.min(10,1+Math.max(0,...(s.expeditionGates||[]).map(Number))))}
...
function professionRecipeKnown(r){
  return Math.max(1,Number(r?.[4])||1)<=professionUnlockedTier();
}
...
function renderMealProfession(){return renderTieredProfessionGroups(Object.entries(MEALS).filter(([,m])=>(m.tier||1)<=professionUnlockedTier()).map(([id,m])=>({tier:m.tier||1,name:m.name,level:m.level||1,craftable:canPayMaterials(m.cost)&&professionState('cooking').level>=m.level&&professionWorkerAvailable('cooking'),html:()=>renderMealRecipe(id,m)})))}
...
function renderCraft(){
  ...
  const filterButton=$('recipeMenuButton');if(filterButton)filterButton.textContent=({all:'All',Weapon:'Weapons',Armor:'Armor',OffHand:'Off-hand',Accessories:'Accessories',Material:'Materials'}[recipeFilter]||'All')+' ▾';
