...
function harness(saved={}){
 ...
 const src=fs.readFileSync(__dirname+'/game.js','utf8').replace(/\}\)\(\);\s*$/,`globalThis.test={start:()=>{start();enter()},startTown:start,stageCount,shopStock,setStage:n=>stage=n,clearRegion:()=>{for(let i=stage;i<stageCount();){completeArea();if(state==='map')break}},openMap,travel,completeArea,healTown,sellItem,buyRune,pickPotions,unlockedNodes,inspect,inspectHero,restoreInfo,update,damage,xp,equip,enter,load,save,stats,allocate,select,moveItem,attackToken,basicHit,activate,tickEffects,enemy,shoot,rollDrop,stepBody,resolveStrike,frame,toggle,get:()=>({heroes,enemies,area,stage,state,inventory,inventoryRunes,fields,shots,loot,paused,time,completed,potions,currentNode,selected,hoverHero,inspectingItem,gold}),setPotion:p=>potions.push(p),setGold:n=>gold=n,setPicked:s=>pickedSlot=s,setGearDrag:v=>gearDrag=v,setArea:n=>area=n,setItem:(i,id)=>inventory[i]=id,setParty:ids=>heroes=ids.map(hero),items:ITEMS,effects:EFFECTS};})();`);
...
t.get().enemies.slice().forEach(e=>t.damage(e,99999));t.update(.01);assert.equal(t.get().state,'walk');assert.equal(t.get().area,0);for(let i=0;i<1600&&t.get().stage===0;i++)t.update(.02);assert.equal(t.get().stage,1);assert.ok(!t.get().completed.includes('a0'));
...
t.setArea(0);t.setParty([0,2,3,4]);t.enter();for(let i=0;i<20000&&t.get().state!=='map'&&t.get().state!=='lost';i++)t.update(.02);assert.ok(t.get().stage>0);assert.ok(t.get().heroes.every(h=>h.sp>0&&h.int===0));
...
console.log('PASS: permanent weapon sockets, 5–8 stages and bosses, hidden map nodes, gated shop stock,  map locks/branches, town healing, trader buy/sell, persistent HP, no level-up healing, 5% tactical potions, left-panel hover, and all combat/rune regressions.');
