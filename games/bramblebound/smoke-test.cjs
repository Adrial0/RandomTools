...
function harness(saved={}){
 ...
 const src=fs.readFileSync(__dirname+'/game.js','utf8').replace(/\}\)\(\);\s*$/,`globalThis.test={start:()=>{start();enter()},startTown:start,openMap,travel,completeArea,healTown,sellItem,buyRune,pickPotions,unlockedNodes,inspect,inspectHero,restoreInfo,update,damage,xp,equip,enter,load,save,stats,allocate,select,moveItem,attackToken,basicHit,activate,tickEffects,enemy,shoot,rollDrop,stepBody,resolveStrike,frame,toggle,get:()=>({heroes,enemies,area,state,inventory,fields,shots,loot,paused,time,completed,potions,currentNode,selected,hoverHero,inspectingItem,gold}),setPotion:p=>potions.push(p),setGold:n=>gold=n,setPicked:s=>pickedSlot=s,setGearDrag:v=>gearDrag=v,setArea:n=>area=n,setItem:(i,id)=>inventory[i]=id,setParty:ids=>heroes=ids.map(hero),items:ITEMS,effects:EFFECTS};})();`);
...
t.get().enemies.slice().forEach(e=>t.damage(e,99999));t.update(.01);assert.equal(t.get().state,'walk');assert.equal(t.get().area,0);for(let i=0;i<1600&&t.get().state!=='map';i++)t.update(.02);assert.ok(t.get().completed.includes('a0'));
...
t.setArea(0);t.setParty([0,2,3,4]);t.enter();for(let i=0;i<20000&&t.get().state!=='map'&&t.get().state!=='lost';i++)t.update(.02);assert.ok(t.get().completed.includes('a0'));assert.ok(t.get().heroes.every(h=>h.sp>0&&h.int===0));
...
const world=harness(),wt=world.t;wt.startTown();assert.equal(wt.get().state,'map');assert.equal(wt.get().currentNode,'town');assert.equal(wt.travel('a3'),false);assert.equal(wt.travel('trader'),false);
const wh=wt.get().heroes[0];wh.hp=17;wt.xp(45);assert.equal(wh.hp,17,'Leveling never heals');wt.allocate('str');assert.equal(wh.hp,17,'Spending LP-increasing stats never heals');assert.ok(wt.travel('a0'));assert.equal(wh.hp,17);wt.completeArea();assert.ok(wt.unlockedNodes().has('a1'));assert.ok(!wt.unlockedNodes().has('a2'));assert.ok(wt.travel('a1'));assert.equal(wh.hp,17);wt.completeArea();wt.travel('a2');wt.completeArea();assert.ok(wt.unlockedNodes().has('a3'));assert.ok(wt.unlockedNodes().has('trader'));assert.equal(wh.hp,17);
...
console.log('PASS: map locks/branches, town healing, trader buy/sell, persistent HP, no level-up healing, 5% tactical potions, left-panel hover, and all combat/rune regressions.');
