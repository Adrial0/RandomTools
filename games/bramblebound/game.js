...
function controlDuration(h,base){return Math.max(.06,Math.min(base,base*h.cool/(85/30)))}
...
function buyPrice(w){if(w.type==='rune')return 80;if(w.type==='gem')return 100*w.tier;if(w.priceIndex!=null)return weaponPrice(w.priceIndex);const order=['basic','iron','fire','lightning','ice','heavy','poison','stun','heal','drain','steel'];return weaponPrice(Math.max(0,order.indexOf(w.id.split('-')[1])))}
