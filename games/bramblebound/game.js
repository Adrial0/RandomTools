...
function update(dt){time+=dt;if(drag)stepHeld(drag,dt);numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 for(const h of heroes){h.flash=Math.max(0,(h.flash||0)-dt);h.anim=Math.max(0,h.anim-dt);resolveStrike(h,dt);if(h.hp<=0||h===drag)continue;stepBody(h,dt);h.hp=Math.min(h.maxHp,h.hp+h.runeBonus.regen*dt);h.cooldown-=dt;h.hold=Math.max(0,h.hold-dt);h.walk=false;if(!grounded(h)){h.strike=null;continue}if(state==='walk'){h.drive=0;continue}if(h.kind==='heal'&&h.cooldown<=0){const ally=heroes.filter(a=>a.hp>0&&a.hp<a.maxHp*.85).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(ally){if(Math.abs(ally.x-h.x)<h.range){shoot(h,ally,'heal',h.healing);h.cooldown=h.cool;attackMotion(h);continue}else if(h.hold<=0){h.drive=Math.sign(ally.x-h.x)*32;h.walk=true;continue}}}
