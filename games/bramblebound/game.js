...
function start(){menuOpen=false;sessionSlot=activeSlot;$('#main-menu').hidden=true;autoMoveEnabled=true;inventoryRunes=Array.from({length:15},()=>[null,null]);stage=0;heroes=[...document.querySelectorAll('#choices select')].map((el,i)=>hero(+el.value,i));area=0;gold=0;inventory=Array(15).fill(null);selected=0;completed=[];currentNode='town';state='town';$('#setup').hidden=true;enterService('town');}
...
function chooseSaveSlot(slot){if(![1,2,3].includes(slot))return;activeSlot=slot;menuOpen=false;$('#main-menu').hidden=true;if(sessionSlot===slot&&state!=='setup')return;
 ...
 sessionSlot=0;state='setup';paused=false;$('#world').hidden=true;$('#services').hidden=true;$('#service-controls').hidden=true;$('#result').hidden=true;$('#setup').hidden=false;tell('Choose your party for save '+slot+'.')
}
$('#menu-button').onclick=openMainMenu;$('#setup-back').onclick=openMainMenu;
$('#save-slots').onclick=e=>{const b=e.target.closest('[data-save-slot]');if(b)chooseSaveSlot(+b.dataset.saveSlot)};
...
function previews(){document.querySelectorAll('#choices select').forEach((el,i)=>{const c=$(`#choice-${i}`).getContext('2d');c.clearRect(0,0,32,40);stick(c,{...hero(+el.value,i),x:16,y:35},false)})}$('#choices').onchange=previews;previews();if(!load())heroes=[0,2,3,4].map(hero);build();menuOpen=true;$('#main-menu').hidden=false;renderSaveSlots();
