...
function enemy(type,x){const boss=type==='boss',tier=Math.floor(area/3);return {id:++uid,type,x,y:floor(x),hp:boss?150+area*65:30+area*12+stage*3,maxHp:boss?150+area*65:30+area*12+stage*3,at:boss?10+area*2:6+area*1.5,range:type==='spitter'||type==='summoner'?105:16,cooldown:.5+(x%7)/10,summon:7,remaining:3,color:boss?'#de6262':type==='summoner'?'#c478ed':type==='spitter'?'#e6b94b':tier===1?'#7ca3ed':'#59df42',seed:x,flash:0,specialCooldown:2+(x%5)*.4,patternIndex:0,warning:null}}
function enter(){currentNode='a'+area;$('#world').hidden=true;mapReturn=null;state='fight';paused=false;drag=null;shots=[];hazards=[];blasts=[];numbers=[];loot=[];potions=[];enemies=[];fields=[];flashes=[];$('#setup').hidden=true;$('#result').hidden=true;heroes.forEach((h,i)=>{h.x=32+i*20;h.y=floor(h.x);h.vy=0;h.vx=0;h.drive=0;h.strike=null;h.hold=0;h.hp=Math.min(h.maxHp,h.hp)});const count=4+area%3+Math.floor(area/3);for(let i=0;i<count;i++){const type=stage===stageCount()-1&&i===count-1?'boss':area>=4&&i===count-1?'summoner':area>0&&i%3===2?'spitter':'slime';enemies.push(enemy(type,240+i*(245/(count-1))))}tell('Drag to position. Defeat the enemies, then walk right.');save();build();}
...
function shoot(h,target,kind,amount,attack=null){if(kind==='enemy'){launchHazard(h,target,'bullet',0,amount);return}shots.push({x:h.x,y:h.y-13,target,kind,amount,attack,element:kind==='enemy'?(h.type==='spitter'?'poison':'lightning'):'physical',life:3})}
...
function launchHazard(e,target,kind,offset=0,amount=e.at*3){
 ...
 const p={x,y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,kind,amount,target,age:0,life:kind==='missile'?4:5,element:kind==='bomb'?'fire':kind==='missile'?'lightning':'physical'};
...
function specialPattern(e){if(e.type==='boss')return PATTERNS[e.patternIndex%4];if(e.type==='summoner')return 'SEEKERS';if(e.type==='spitter')return PATTERNS[(area+stage)%4];return stage>0&&e.seed>400?PATTERNS[stage%4]:null}
function tickSpecial(e,target,dt){
 ...
 if(w.pattern==='BOMBS')for(let i=-1;i<=1;i++)launchHazard(e,aim,'bomb',i*.55,e.at*4);
...
function tickHazards(dt){
 ...
 if(p.kind==='missile'&&p.age<2.1&&p.target.hp>0){const angle=Math.atan2(p.vy,p.vx),goal=Math.atan2(p.target.y-13-p.y,p.target.x-p.x),delta=Math.atan2(Math.sin(goal-angle),Math.cos(goal-angle)),turn=Math.max(-1.5*dt,Math.min(1.5*dt,delta));p.vx=Math.cos(angle+turn)*90;p.vy=Math.sin(angle+turn)*90}
...
function buyPrice(w){return w.type==='rune'?80:30+w.tier*50}
...
function healTown(){if(state!=='service'||currentNode!=='town')return false;heroes.forEach(h=>h.hp=h.maxHp);save();refresh();tell('The inn restored and revived your party.');return true}
...
function update(dt){time+=dt;numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 ...
 const additions=[];for(const e of enemies){if(e.hp<=0||e.frozen>0||e.stun>0)continue;e.flash=Math.max(0,e.flash-dt);e.x=Math.max(8,Math.min(505,e.x+(e.kick||0)*dt));e.kick=(e.kick||0)*Math.exp(-9*dt);e.y=floor(e.x);e.cooldown-=dt;if(e.type==='summoner'){e.summon-=dt;if(e.summon<=0&&e.remaining>0){const m=enemy('slime',Math.max(130,e.x-20));m.minion=true;m.hp=m.maxHp*=.55;additions.push(m);e.remaining--;e.summon=8;float(e.x,e.y-30,'SUMMON','#d795ee')}}const target=heroes.filter(h=>h.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;tickSpecial(e,target,dt);const d=Math.abs(target.x-e.x);if(d>e.range)e.x+=Math.sign(target.x-e.x)*(e.type==='boss'?13:17)*(e.slow>0?.6:1)*dt;else if(e.cooldown<=0&&Math.abs(target.y-e.y)<50){e.cooldown=e.type==='boss'?1.2:1.4;if(e.range>20)shoot(e,target,'enemy',e.at);else damage(target,e.at)}}enemies.push(...additions);
 for(const s of shots){s.life-=dt;if(s.target.hp<=0){s.life=0;continue}const dx=s.target.x-s.x,dy=s.target.y-12-s.y,d=Math.hypot(dx,dy),v=s.kind==='bullet'?330:180;if(d<v*dt+3){if(s.kind==='heal'){const n=Math.min(s.target.maxHp-s.target.hp,s.amount);s.target.hp+=n;float(s.target.x,s.target.y-22,`+${n}`,'#66ff66')}else{if(s.attack)basicHit(s.target,s.amount,s.attack);else damage(s.target,s.amount,s.element)}s.life=0}else{s.x+=dx/d*v*dt;s.y+=dy/d*v*dt}}shots=shots.filter(s=>s.life>0);enemies=enemies.filter(e=>e.hp>0);tickHazards(dt);pickLoot();pickPotions();
 ...
 if(!enemies.length&&!hazards.length&&state==='fight'){state='walk';tell('STAGE CLEAR — moving to the right exit.');save()}
...
function portrait(c,classId){c.clearRect(0,0,32,36);const col=classes[classId].color;c.strokeStyle=col;c.strokeRect(13,4,6,6);line(c,[[16,11],[16,22],[11,31]],col);line(c,[[16,22],[21,31]],col);line(c,[[10,21],[12,15],[16,13],[21,17],[23,13]],col);c.fillStyle=col;c.font='10px monospace';c.fillText(classes[classId].symbol,23,13)}
...
function moveHeld(x,y,t){if(!drag)return;const h=drag,nx=Math.max(8,Math.min(503,x)),ny=Math.max(20,Math.min(floor(nx),y)),prev=h.dragSample,dt=prev?Math.max(.008,(t-prev.t)/1000):.016;
 h.throwVx=Math.max(-420,Math.min(420,(nx-h.x)/dt));h.throwVy=Math.max(-480,Math.min(480,(ny-h.y)/dt));h.dragSample={x:nx,y:ny,t};h.x=nx;h.y=ny;h.vx=h.vy=0;h.strike=null}
function release(e){if(drag){const age=e&&Number.isFinite(e.timeStamp)?Math.max(0,e.timeStamp-(drag.dragSample?.t||0)):1000,decay=Math.max(0,1-age/120);drag.vx=(drag.throwVx||0)*decay;drag.vy=(drag.throwVy||0)*decay;drag.hold=.4;drag.drive=0;drag.dragSample=null;drag=null}}
