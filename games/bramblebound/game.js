...
function enemy(type,x){const boss=type==='boss',tier=Math.floor(area/3),hp=Math.round(areaHealth(area)*(boss?10:type==='summoner'?1.25:1));return {id:++uid,type,x,y:floor(x),hp:hp,maxHp:hp,at:boss?10+area*2:6+area*1.5,range:type==='spitter'||type==='summoner'?105:type==='slasher'?(area<3?22:48):(area<3?12:16),cooldown:.5+(x%7)/10,summon:7,remaining:3,color:boss?'#de6262':type==='summoner'?'#c478ed':type==='slasher'?'#f37c52':type==='spitter'?'#e6b94b':tier===1?'#7ca3ed':'#59df42',seed:x,flash:0,specialCooldown:2+(x%5)*.4,patternIndex:0,warning:null}}
