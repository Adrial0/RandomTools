...
function encounterType(i,count){if(stage===stageCount()-1&&i===count-1)return 'boss';if(area>=4&&i===count-1)return 'summoner';if(area===0&&stage===0)return i%2?'slasher':'slime';const pool=area<3?['slime','swarmling','hopper','slasher','beetle']:area<6?['archer','swarmling','shaman','slasher','hopper','bomber']:['beetle','bomber','shaman','archer','slasher','spitter'];return pool[(i+stage+area)%pool.length]}
...
function launchHazard(e,target,kind,offset=0,amount=e.at*3){
 ...
 if(kind==='arrow'||kind==='bomb'){const flight=kind==='bomb'?1.9:.85;p.vx=dx/flight+offset*100;p.gravity=kind==='bomb'?100:240;p.vy=dy/flight-.5*p.gravity*flight;if(kind==='bomb')p.fuse=2.6}
 hazards.push(p);return p;
}
function enemyTrait(e,target,dt){
 if(e.type==='hopper'){e.hopCooldown-=dt;if(e.hopCooldown<.45&&e.hopCooldown>0)e.crouching=true;if(e.hopCooldown<=0&&Math.abs(e.y-floor(e.x))<1){e.vy=-155;e.kick=Math.sign(target.x-e.x)*95;e.hopCooldown=2.8;e.crouching=false;float(e.x,e.y-26,'HOP','#e1b75b')}}
...
function update(dt){time+=dt;if(drag)stepHeld(drag,dt);numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}uiTime+=dt;if(uiTime>.1){refresh();uiTime=0}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 ...
 const additions=[];for(const e of enemies){if(e.hp<=0||e.frozen>0||e.stun>0)continue;tickSlash(e,dt);e.flash=Math.max(0,e.flash-dt);moveEnemy(e,(e.kick||0)*dt,dt);e.kick=(e.kick||0)*Math.exp(-9*dt);e.cooldown-=dt;if(e.type==='summoner'){e.summon-=dt;if(e.summon<=0&&e.remaining>0){const m=enemy('slime',Math.max(130,e.x-20));m.minion=true;m.hp=m.maxHp*=.55;additions.push(m);e.remaining--;e.summon=8;float(e.x,e.y-30,'SUMMON','#d795ee')}}const target=heroes.filter(h=>h.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;enemyTrait(e,target,dt);tickSpecial(e,target,dt);const d=Math.abs(target.x-e.x);if(e.type==='archer'&&d<65)moveEnemy(e,-Math.sign(target.x-e.x)*e.speed*dt,0);else if(d>e.range)moveEnemy(e,Math.sign(target.x-e.x)*e.speed*(e.slow>0?.6:1)*dt,0);else if(e.type!=='bomber'&&e.cooldown<=0&&Math.abs(target.y-e.y)<(e.range>50?50:18)){e.cooldown=e.type==='boss'?1.2:1.4;if(e.type==='slasher'){e.slash={left:.18,face:Math.sign(target.x-e.x)||1,hit:false,life:.32};e.cooldown=1.6}else if(e.type==='archer')launchHazard(e,target,'arrow',0,e.at);else if(e.range>20)shoot(e,target,'enemy',e.at);else damage(target,e.at)}}enemies.push(...additions);
...
function drawEnemy(e){const r=e.type==='boss'?17:e.type==='beetle'?11:e.type==='swarmling'?4:e.type==='summoner'?9:7,x=Math.round(e.x),y=Math.round(e.y)+(e.crouching?3:0),color=e.flash?'#fff':e.frozen>0?'#79cfff':e.poison?'#a3ff57':e.color;ctx.strokeStyle=color;ctx.fillStyle=color;ctx.beginPath();
 if(e.type==='summoner'||e.type==='shaman'){ctx.moveTo(x-r,y-3);ctx.lineTo(x,y-25);ctx.lineTo(x+r,y-3);ctx.closePath();ctx.stroke();if(e.type==='shaman'){line(ctx,[[x+12,y],[x+12,y-25]],color);ctx.strokeRect(x+10,y-28,4,4)}}
