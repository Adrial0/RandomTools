...
function update(dt){time+=dt;if(drag)stepHeld(drag,dt);numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 for(const h of heroes){h.flash=Math.max(0,(h.flash||0)-dt);h.anim=Math.max(0,h.anim-dt);resolveStrike(h,dt);if(h.hp<=0||h===drag)continue;stepBody(h,dt);h.hp=Math.min(h.maxHp,h.hp+h.runeBonus.regen*dt);h.cooldown-=dt;h.hold=Math.max(0,h.hold-dt);h.walk=false;if(!grounded(h)){h.strike=null;continue}if(state==='walk'){h.drive=39;h.walk=true;continue}if(h.kind==='heal'&&h.cooldown<=0){const ally=heroes.filter(a=>a.hp>0&&a.hp<a.maxHp*.85).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(ally){if(Math.abs(ally.x-h.x)<h.range){shoot(h,ally,'heal',h.healing);h.cooldown=h.cool;attackMotion(h);continue}else if(h.hold<=0){h.drive=Math.sign(ally.x-h.x)*32;h.walk=true;continue}}}
...
function stepHeld(h,dt){if(!h.dragTarget)return;const target=h.dragTarget;h.vx=(h.vx||0)+((target.x-h.x)*110-(h.vx||0)*15)*dt;h.vy=(h.vy||0)+((target.y-h.y)*110-(h.vy||0)*15+100)*dt;h.vx=Math.max(-420,Math.min(420,h.vx));h.vy=Math.max(-480,Math.min(480,h.vy));h.x=Math.max(8,Math.min(503,h.x+h.vx*dt));h.y=Math.max(8,Math.min(floor(h.x),h.y+h.vy*dt));h.leanV=(h.leanV||0)+((h.vx*.06-(h.lean||0))*70-(h.leanV||0)*8)*dt;h.lean=(h.lean||0)+h.leanV*dt;h.swingV=(h.swingV||0)+(-h.swing*70-(h.swingV||0)*7-h.vx*.15)*dt;if(!Number.isFinite(h.swingV))h.swingV=0;h.swing=(h.swing||0)+h.swingV*dt;springPose(h,dt)}
...
function moveHeld(x,y,t){if(!drag)return;drag.dragTarget={x:Math.max(8,Math.min(503,x)),y:Math.max(20,Math.min(floor(x),y))};drag.dragSample={x,y,t};drag.strike=null}
