...
function save(){if(!heroes.length)return;try{localStorage.setItem(KEY,JSON.stringify({version:4,area,gold,inventory,heroes:heroes.map(h=>({classId:h.classId,level:h.level,xp:h.xp,weapon:h.weapon,runes:h.runes,attributes:h.attributes,sp:h.sp,mp:h.mp}))}))}catch{}}
function load(){try{
...
function inspect(id){const w=ITEMS[id],e=EFFECTS[w?.effect];$('#item-name').textContent=w?w.name:'Empty slot';if(w?.type==='rune'){$('#item-details').textContent='Rune artifact · All classes · LV '+w.level+' · Equip in either RUNE slot';$('#item-effect').textContent=w.description+' Two runes stack additively. Elemental resistance caps at 75%.';return}$('#item-details').textContent=w?classes[w.classId].name+' · LV '+w.level+' · AT '+w.min+'–'+w.max+' · AGI '+w.agi.join('–')+' · RANGE '+w.range+' · '+(e?'MP '+e.mp+' · '+e.name:'Physical / no ability'):'Drag a matching item here to equip or store it.';$('#item-effect').textContent=e?e.description+' Bonus AT '+e.min+'–'+e.max+'. MP gained only by successful basic hits.':w?'Higher physical AT can be a better choice than a weaker elemental weapon.':'';}
function refresh(){const h=heroes[selected];if(!h)return;$('#class-name').textContent=classes[h.classId].name;$('#lp').textContent=Math.ceil(h.hp)+'/'+h.maxHp;$('#attack').textContent=h.atMin+'–'+h.atMax;$('#agi').textContent=h.agi.join('–');$('#range').textContent=h.range;$('#level').textContent=h.level;$('#sp').textContent=h.sp;for(const [i,name] of ['str','dex','int'].entries()){$('#'+name).textContent=h[name];const btn=$('[data-stat="'+name+'"]');btn.disabled=h.sp<1;btn.title=STAT_HELP[h.classId][i];btn.setAttribute('aria-label','Add '+name.toUpperCase()+': '+STAT_HELP[h.classId][i]);}
 ...
 $('#xp').textContent=Math.floor(h.xp)+'/'+needed(h.level);$('#xp-fill').style.width=h.xp/needed(h.level)*100+'%';$('#gold').textContent=gold;$('#area').textContent=areas[Math.floor(area/3)]+': '+(area%3+1);$('#pause').textContent=paused?'Resume':'Pause';heroes.forEach((h,i)=>{const bar=$('[data-hero="'+i+'"] .life i');if(bar)bar.style.width=Math.max(0,h.hp/h.maxHp)*100+'%'});}
...
canvas.addEventListener('pointermove',e=>{if(!drag||paused)return;const p=point(e);drag.x=Math.max(8,Math.min(503,p.x));drag.y=Math.max(45,Math.min(floor(drag.x),p.y+13));drag.vy=0;drag.vx=0;drag.strike=null});function release(){if(drag){drag.hold=.3;drag.drive=0;drag=null}}canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);
