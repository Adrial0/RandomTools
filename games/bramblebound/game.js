...
function update(dt){time+=dt;if(drag)stepHeld(drag,dt);numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 ...
 const additions=[];for(const e of enemies){if(e.hp<=0||e.frozen>0||e.stun>0)continue;e.flash=Math.max(0,e.flash-dt);moveEnemy(e,(e.kick||0)*dt,dt);e.kick=(e.kick||0)*Math.exp(-9*dt);e.cooldown-=dt;if(e.type==='summoner'){e.summon-=dt;if(e.summon<=0&&e.remaining>0){const m=enemy('slime',Math.max(130,e.x-20));m.minion=true;m.hp=m.maxHp*=.55;additions.push(m);e.remaining--;e.summon=8;float(e.x,e.y-30,'SUMMON','#d795ee')}}const target=heroes.filter(h=>h.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;tickSpecial(e,target,dt);const d=Math.abs(target.x-e.x);if(d>e.range)moveEnemy(e,Math.sign(target.x-e.x)*(e.type==='boss'?13:17)*(e.slow>0?.6:1)*dt,0);else if(e.cooldown<=0&&Math.abs(target.y-e.y)<50){e.cooldown=e.type==='boss'?1.2:1.4;if(e.range>20)shoot(e,target,'enemy',e.at);else damage(target,e.at)}}enemies.push(...additions);
...
function drawWeapon(c,id,hand,f,arm,color){c.save();c.translate(hand[0],hand[1]);c.scale(f,1);c.strokeStyle=color;c.fillStyle=color;
 if(id===0){c.rotate(arm*.45);line(c,[[0,3],[0,-13],[2,-16],[3,-13],[2,3]],color);line(c,[[-4,-1],[5,-1]],'#c9a459');line(c,[[1,0],[1,5]],'#aa7848')}
 ...
 if(id===5){c.rotate(arm*.35);line(c,[[-5,8],[12,-18]],'#bd965e');line(c,[[9,-15],[12,-22],[14,-17],[9,-15]],color)}
