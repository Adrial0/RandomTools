...
function enemy(type,x){const boss=type==='boss',tier=Math.floor(area/3);return {id:++uid,type,x,y:floor(x),hp:boss?150+area*65:30+area*12+stage*3,maxHp:boss?150+area*65:30+area*12+stage*3,at:boss?7+area*2:4+area,range:type==='spitter'||type==='summoner'?105:16,cooldown:.5+(x%7)/10,summon:7,remaining:3,color:boss?'#de6262':type==='summoner'?'#c478ed':type==='spitter'?'#e6b94b':tier===1?'#7ca3ed':'#59df42',seed:x,flash:0}}
...
function enterService(id){currentNode=id;mapReturn=null;state='service';paused=false;drag=null;shots=[];fields=[];flashes=[];numbers=[];loot=[];enemies=[];potions=[];shopIndex=0;$('#world').hidden=true;$('#result').hidden=true;$('#services').hidden=true;heroes.forEach((h,i)=>{h.x=130+i*20;h.y=226;h.vx=h.vy=h.drive=0;h.strike=null});$('#service-controls').hidden=false;$('#inn').hidden=id!=='town';save();build();tell('Visit the shop, rest at the inn, or open the world map.')}
...
function update(dt){time+=dt;numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 ...
 if(!enemies.length&&state==='fight'){state='walk';tell('STAGE CLEAR — moving to the right exit.');save()}
