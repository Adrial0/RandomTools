...
function buyPrice(w){if(w.type==='rune')return 80;if(w.type==='gem')return 100*w.tier;return weaponPrice(w.tier)}
function shopPages(){const stock=shopStock();return [...new Set(stock.map(w=>w.tier))].sort((a,b)=>a-b).map(tier=>({tier,items:stock.filter(w=>w.tier===tier)}))}
function renderServices(){if(state!=='service'){$('#services').hidden=true;return}const pages=shopPages();shopIndex=Math.max(0,Math.min(shopIndex,pages.length-1));const page=pages[shopIndex];$('#service-title').textContent=currentNode==='town'?'TOWN SHOP':'RUNE TRADER';$('#service-description').textContent='Choose a category. Arrows change tier.';$('#town-heal').hidden=true;const id=pickedSlot?.type==='bag'?slotItem(pickedSlot):null;$('#sell-item').disabled=!id;$('#sell-item').textContent=id?'Sell '+ITEMS[id].name+' — '+salePrice(id)+' gold':'Select inventory item to sell';$('#shop-classes').hidden=currentNode!=='town';$('#shop-classes').innerHTML=classes.map((c,i)=>'<button data-shop-class="'+i+'" class="'+(shopClass===i?'active':'')+'" aria-label="Shop '+c.name+'">'+c.symbol+'</button>').join('');$('#trader-stock').innerHTML=(page?.items||[]).map(w=>'<div class="shop-card"><strong>'+w.name+'</strong><div>'+(socketItem(w.id)?w.description:'AT '+w.min+'–'+w.max+' · AGI '+w.agi.join('–')+'<br>RANGE '+w.range+' · '+(w.effect||'Physical')+(w.arrows?' · '+w.arrows+' arrows':''))+'</div><button data-buy="'+w.id+'" '+(gold<buyPrice(w)||!inventory.includes(null)?'disabled':'')+'>BUY · '+buyPrice(w)+' gold</button></div>').join('');$('#shop-page').textContent=page?'Tier '+(currentNode==='town'?page.tier+1:page.tier)+' · '+(shopIndex+1)+' / '+pages.length:'No stock';$('#shop-prev').disabled=shopIndex<=0;$('#shop-next').disabled=shopIndex>=pages.length-1}
