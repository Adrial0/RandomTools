...
function encounterType(i,count){if(stage===stageCount()-1&&i===count-1)return 'guardian'+area;const region=Math.floor(area/3);if(area>=4&&i===count-1)return REGION_SUMMONERS[region];if(area===0&&stage===0)return i%2?'slasher':'slime';const roster=REGIONAL_POOLS[region],offset=(area%3)*2,pool=Array.from({length:5},(_,n)=>roster[(offset+n)%roster.length]);return pool[(i+stage)%pool.length]}
...
function tickShots(dt){for(const s of shots){if(s.summonOwner&&(s.summonOwner.hp<=0||s.summonOwner.gearRevision!==s.summonRevision)){s.life=0;continue}s.life-=dt;const ax=s.x,ay=s.y;s.vy+=(s.gravity||0)*dt;s.x+=s.vx*dt;s.y+=s.vy*dt;const wall=terrainHit(ax,ay,s.x,s.y),bx=wall?wall.x:s.x,by=wall?wall.y:s.y;
 if(s.kind==='note'){
  ...
  const hits=enemies.filter(e=>e.hp>0&&!s.hitSet.has(e)&&segmentDistance(e.x,e.y-10,ax,ay,bx,by)<(e.type==='boss'?16:7)+(s.radius||0));
 ...
 }else{const hit=enemies.filter(e=>e.hp>0&&segmentDistance(e.x,e.y-10,ax,ay,bx,by)<(e.type==='boss'?16:7)).sort((a,b)=>Math.hypot(a.x-ax,a.y-10-ay)-Math.hypot(b.x-ax,b.y-10-ay))[0];if(hit){if(s.summonOwner&&s.physical)s.amount+=songTotal(hit,'vulnerability');if(s.attack)basicHit(hit,s.amount,s.attack);else damage(hit,s.amount);s.life=0}}
...
function rollDrops(target){
 ...
 if(Math.random()<Math.min(1,.01*boost)){const pool=Object.values(ITEMS).filter(w=>w.type==='rune'||w.type==='gem'&&w.tier<=1+Math.floor(area/2));result.push(choose(pool).id);}
 if(boss&&Math.random()<Math.min(1,.10*boost)){const tier=1+Math.floor(area/3),pool=Object.values(ITEMS).filter(w=>w.type==='soul'&&w.tier===tier);result.push(choose(pool).id);}
 return result;
}
...
function applySong(target,owner,mod={}){
 ...
 target.songs[owner.id]={until:time+(mod.duration||3),attack:owner.str*power,haste:owner.dex*.01*power,weakness:owner.str*.5*power,vulnerability:owner.dex*.25*power};
}
...
function aura(h){let attack=1,defense=0;for(const p of heroes)if(p.hp>0&&p.classId===4&&Math.hypot(p.x-h.x,p.y-h.y)<=p.range){attack+=p.str*.01;defense+=p.dex*.2}return {attack,defense,flat:songTotal(h,'attack')}}
...
function basicHit(target,amount,attack){if(target.hp<=0)return;const before=target.hp;damage(target,amount+(['melee','arrow'].includes(attack?.owner?.kind)?songTotal(target,'vulnerability'):0));const owner=attack?.owner;if(owner?.hp>0&&owner.gearRevision===attack.revision)owner.hp=Math.min(owner.maxHp,owner.hp+(before-target.hp)*owner.runeBonus.lifesteal+(owner.classId===5?owner.dex*.5:0));if(!attack||attack.charged||owner?.classId===7)return;attack.charged=true;const h=attack.owner,w=ITEMS[attack.weapon],effect=EFFECTS[w?.effect];if(!effect||h.hp<=0||h.weapon!==attack.weapon||h.gearRevision!==attack.revision)return;h.mp+=h.int;if(h.mp>=mpCost(w)){h.mp=0;activate(h,target,w.effect)}save()}
...
function activate(h,target,kind){if(['encore','sustain','crescendo','restore'].includes(kind)){if(kind==='encore')shootNote(h,target,basicAmount(h),{...attackToken(h),charged:true});else h.nextNote=kind==='sustain'?{duration:6}:kind==='crescendo'?{power:1.5}:{heal:12};return}const e=EFFECTS[kind],power=h.abilityPower*.65,amount=()=>Math.max(1,Math.round(roll(e.min,e.max)*power));float(h.x,h.y-38,e.name,e.color);flashes.push({x:h.x,y:h.y-18,tx:target.x,ty:target.y-10,color:e.color,life:.35,kind});
...
function stageExitOpen(){return stage<stageCount()-1||!enemies.some(e=>e.type==='boss'&&e.hp>0)}
function completeArea(){if(!stageExitOpen())return false;if(stage<stageCount()-1){stage++;enter();return}const id='a'+area;if(!completed.includes(id))completed.push(id);mapReturn=null;state='map';openMap();tell('Boss defeated. New routes discovered.');save()}
...
function update(dt){time+=dt;if(drag)stepHeld(drag,dt);numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){heroes.forEach(h=>tickRogueArms(h,dt));if(heroes.some(h=>h.hp>0&&h.x+6>=538&&h.x-6<=572&&h.y>=207&&h.y-27<=226)){openMap();return}for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}uiTime+=dt;if(uiTime>.1){refresh();uiTime=0}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);tickMinions(dt);
 ...
 const target=enemies.filter(e=>e.hp>0).sort((a,b)=>Math.hypot(a.x-h.x,a.y-h.y)-Math.hypot(b.x-h.x,b.y-h.y))[0];if(!target)continue;if(h.classId===7){h.face=target.x>=h.x?1:-1;h.drive=Math.abs(target.x-h.x)>h.range&&h.hold<=0?h.face*33:0;h.walk=!!h.drive;continue}const d=Math.hypot(target.x-h.x,target.y-h.y);h.face=target.x>=h.x?1:-1;if(d>h.range&&h.hold<=0){h.drive=h.face*33;h.walk=true}if(d<=h.range+3&&(h.kind==='aura'||Math.abs(h.y-target.y)<45)&&h.cooldown<=0){h.cooldown=roll(...effectiveAgi(h))/30;attackMotion(h);const token=attackToken(h),amount=basicAmount(h);if(h.kind==='aura'){for(const e of enemies)if(e.hp>0&&Math.hypot(e.x-h.x,e.y-h.y)<=h.range)basicHit(e,amount,token);h.auraFlash=.25}else if(h.kind==='melee')h.strike={left:.09,target,amount,token,range:h.range,elapsed:0,hits:new Set(),face:h.face};else shoot(h,target,h.kind==='heal'?'magic':h.kind,amount,token)}}
 const additions=[];for(const e of enemies){if(e.hp<=0||e.frozen>0||e.stun>0)continue;tickSlash(e,dt);e.flash=Math.max(0,e.flash-dt);if(!physicalEnemy(e))moveEnemy(e,(e.kick||0)*dt,dt);e.kick=(e.kick||0)*Math.exp(-9*dt);e.cooldown-=dt;if(e.type==='summoner'){e.summon-=dt;if(e.summon<=0&&e.remaining>0){const m=enemy(REGIONAL_POOLS[Math.floor(area/3)][(e.remaining+area)%7],Math.max(130,e.x-20));m.minion=true;m.hp=m.maxHp*=.55;additions.push(m);e.remaining--;e.summon=8;float(e.x,e.y-30,'SUMMON','#d795ee')}}const target=combatAllies().filter(h=>h.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;if(physicalEnemy(e))stepEnemyPhysics(e,target,dt);tickCatapult(e,target,dt);enemyTrait(e,target,dt);tickSpecial(e,target,dt);const d=Math.abs(target.x-e.x);if(e.type==='archer'&&d<65)moveEnemy(e,-Math.sign(target.x-e.x)*e.speed*dt,0);else if(d>e.range&&!physicalEnemy(e))moveEnemy(e,Math.sign(target.x-e.x)*e.speed*(slowFactor(e))*dt,0);else if(d<=e.range&&e.type!=='bomber'&&e.type!=='catapult'&&e.cooldown<=0&&Math.abs(target.y-e.y)<(e.range>50?50:18)){e.cooldown=e.type==='boss'?1.2:1.4;if(e.type==='slasher'){e.slash={left:.18,face:Math.sign(target.x-e.x)||1,hit:false,life:.32};e.cooldown=1.6}else if(e.type==='archer')launchHazard(e,target,'arrow',0,enemyDamage(e));else if(e.range>20)shoot(e,target,'enemy',enemyDamage(e));else damage(target,enemyDamage(e))}}enemies.push(...additions);
 tickShots(dt);enemies=enemies.filter(e=>e.hp>0);tickHazards(dt);pickLoot();pickPotions();
 if(heroes.every(h=>h.hp<=0)){heroes.forEach(h=>h.hp=Math.max(1,Math.ceil(h.maxHp*.05)));enterService('town');tell('Party defeated. Returned to town with 5% HP.');return}
...
function tickMinions(dt){
 ...
 for(const m of minions){
  ...
  const amount=Math.max(1,Math.round((roll(h.atMin,h.atMax)*spec.damage+songTotal(m,'attack'))));
...
function terrain(){ctx.fillStyle='#000';ctx.fillRect(0,0,576,256);const biome=state==='service'?0:Math.floor(area/3),cave=biome===2,palette=[['#c99449','#42ed28','#f1c270'],['#695d36','#7aa544','#95804b'],['#837568','#aaa6a0','#b1a092'],['#b88b42','#f1cf78','#e3b965'],['#555c67','#a6abb3','#808995'],['#849cad','#effaff','#bcd8e8']][biome]||['#849cad','#effaff','#bcd8e8'];for(let x=0;x<576;x++){const y=floor(x);ctx.fillStyle=palette[0];ctx.fillRect(x,y,1,256-y);ctx.fillStyle=palette[1];ctx.fillRect(x,y,1,1);ctx.fillStyle=palette[2];for(let py=y+3+(x%3);py<256;py+=4)if(x%3===0)ctx.fillRect(x,py,1,1)}if(biome>=3){for(let i=0;i<6;i++){const x=45+i*83,y=floor(x);if(biome===3){line(ctx,[[x,y],[x,y-25],[x-6,y-20],[x-6,y-13]],'#758344')}else{line(ctx,[[x-27,y],[x,y-65-(i%3)*15],[x+32,y]],biome===5?'#688597':'#414750');if(biome===5)line(ctx,[[x-6,y-51-(i%3)*15],[x,y-65-(i%3)*15],[x+7,y-50-(i%3)*15]],'#e2f3ff')}}}if(cave){ctx.fillStyle='#6c635b';for(let x=0;x<576;x+=8)ctx.fillRect(x,0,8,7+(x*7)%17)}if(['fight','walk'].includes(state)){ctx.fillStyle=stageExitOpen()?'#be8d46':'#74574a';ctx.fillRect(543,205,31,10);ctx.fillRect(548,215,2,11);ctx.fillStyle='#000';ctx.font='8px monospace';ctx.fillText('NEXT>',544,213)}}
...
function drawRegionalEnemy(e,x,y,color){
 ...
 ctx.save();if(boss){ctx.translate(x,y);ctx.scale(1.65,1.65);ctx.translate(-x,-y)}
