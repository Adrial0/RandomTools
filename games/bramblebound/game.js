...
function stats(h){
 ...
 h.str=str;h.dex=dex;h.int=int;h.maxHp=gem.lp+c.hp+(h.level-1)*6+a.str*[4,5,3,2,3,4,3,3][h.classId]+a.dex*[4,3,3,2,3,3,2,3][h.classId]+int*2;
...
function stepEnemyPhysics(e,target,dt){
 const direction=Math.sign(target.x-e.x),distance=Math.abs(target.x-e.x),slow=e.slow>0?.6:1,ground=e.y>=floor(e.x)-.5;
...
function moveItem(from,to){if(!validSlot(from)||!validSlot(to)||from.type===to.type&&from.index===to.index)return false;const a=slotItem(from),b=slotItem(to);if(from.type.startsWith('rune')){tell('Runes and gems are permanently socketed to this weapon.');return false}if(!a)return false;if(!accepts(to,a)||!accepts(from,b)){tell('Cannot equip: weapon class or required level does not match.');return false}
...
function buyPrice(w){if(w.type==='rune')return 80;if(w.type==='gem')return 100*w.tier;const order=['basic','iron','fire','lightning','ice','heavy','poison','stun','heal','drain','steel'];return weaponPrice(Math.max(0,order.indexOf(w.id.split('-')[1])))}
function renderServices(){if(state!=='service'){$('#services').hidden=true;return}const stock=shopStock();shopIndex=Math.max(0,Math.min(shopIndex,stock.length-1));const w=stock[shopIndex];$('#service-title').textContent=currentNode==='town'?'TOWN SHOP':'RUNE TRADER';$('#service-description').textContent='Runes and gems bind permanently. Stock follows cleared areas.';$('#town-heal').hidden=true;const id=pickedSlot?.type==='bag'?slotItem(pickedSlot):null;$('#sell-item').disabled=!id;$('#sell-item').textContent=id?'Sell '+ITEMS[id].name+' — '+salePrice(id)+' gold':'Select inventory item to sell';$('#shop-classes').hidden=currentNode!=='town';$('#shop-classes').innerHTML=classes.map((c,i)=>'<button data-shop-class="'+i+'" class="'+(shopClass===i?'active':'')+'" aria-label="Shop '+c.name+'">'+c.symbol+'</button>').join('');$('#trader-stock').innerHTML=w?'<strong>'+w.name+'</strong><p>'+(socketItem(w.id)?w.description:'AT '+w.min+'–'+w.max+' · AGI '+w.agi.join('–')+' · RANGE '+w.range+'<br>TYPE '+(w.effect||'Physical'))+'</p><button data-buy="'+w.id+'" '+(gold<buyPrice(w)||!inventory.includes(null)?'disabled':'')+'>BUY · '+buyPrice(w)+' gold</button>':'';$('#shop-page').textContent=(shopIndex+1)+' / '+stock.length}
