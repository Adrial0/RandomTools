...
function update(dt){if(sceneFade)return;time+=dt;if(drag)stepHeld(drag,dt);numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state==='service'){heroes.forEach(h=>tickRogueArms(h,dt));if(heroes.some(h=>h.hp>0&&h.x+6>=538&&h.x-6<=572&&h.y>=207&&h.y-27<=226)){changeScene(openMap);return}for(const h of heroes)if(h.hp>0&&h!==drag){h.drive=0;stepBody(h,dt)}uiTime+=dt;if(uiTime>.1){refresh();uiTime=0}return}if(state!=='fight'&&state!=='walk')return;tickEffects(dt);tickMinions(dt);
...
function frame(t){const dt=Math.min(.1,Math.max(0,(t-last)/1000));last=t;tickSceneFade(dt);if(state==='map')panMap(dt);if(!paused&&!sceneFade){let left=dt*speed;while(left>0){const step=Math.min(left,1/120);update(step);left-=step}}draw();requestAnimationFrame(frame)}requestAnimationFrame(frame);
})();
