...
function save(){if(!heroes.length)return;try{localStorage.setItem(KEY,JSON.stringify({version:5,area,gold,inventory,completed,currentNode,heroes:heroes.map(h=>({classId:h.classId,level:h.level,xp:h.xp,hp:h.hp,weapon:h.weapon,runes:h.runes,attributes:h.attributes,sp:h.sp,mp:h.mp}))}))}catch{}}
function load(){try{
...
function enter(){currentNode='a'+area;$('#world').hidden=true;mapReturn=null;state='fight';paused=false;drag=null;shots=[];numbers=[];loot=[];potions=[];enemies=[];fields=[];flashes=[];$('#setup').hidden=true;$('#result').hidden=true;heroes.forEach((h,i)=>{h.x=32+i*20;h.y=floor(h.x);h.vy=0;h.vx=0;h.drive=0;h.strike=null;h.hold=0;h.hp=Math.min(h.maxHp,h.hp)});const count=4+area%3+Math.floor(area/3);for(let i=0;i<count;i++){const type=area===8&&i===count-1?'boss':area>=4&&i===count-1?'summoner':area>0&&i%3===2?'spitter':'slime';enemies.push(enemy(type,240+i*(245/(count-1))))}tell('Drag to position. Defeat the enemies, then walk right.');save();build();}
function start(){heroes=[...document.querySelectorAll('#choices select')].map((el,i)=>hero(+el.value,i));area=0;gold=0;inventory=Array(15).fill(null);selected=0;completed=[];currentNode='town';state='town';$('#setup').hidden=true;openMap();build();}
...
function shoot(h,target,kind,amount,attack=null){shots.push({x:h.x,y:h.y-13,target,kind,amount,attack,element:kind==='enemy'?(h.type==='spitter'?'poison':'lightning'):'physical',life:3})}
...
function accepts(slot,id){if(!id)return true;const item=ITEMS[id];if(!item)return false;if(slot.type==='bag')return true;const h=heroes[slot.index];return item.level<=h.level&&(slot.type==='gear'?item.type==='weapon'&&item.classId===h.classId:item.type==='rune')}
function moveItem(from,to){if(!validSlot(from)||!validSlot(to)||from.type===to.type&&from.index===to.index)return false;const a=slotItem(from),b=slotItem(to);if(!a)return false;if(!accepts(to,a)||!accepts(from,b)){tell('Cannot equip: weapon class or required level does not match.');return false}
 for(const [slot,id] of [[from,b],[to,a]]){if(slot.type==='bag')inventory[slot.index]=id;else{const h=heroes[slot.index];if(slot.type==='gear')h.weapon=id;else h.runes[slot.type==='rune0'?0:1]=id;h.mp=0;h.gearRevision++}}
...
function openMap(){if(state==='setup')return;if(state==='fight'||state==='walk'){mapReturn=state;release()}state='map';$('#world').hidden=false;$('#result').hidden=true;renderMap();refresh();save()}
function renderMap(){const unlocked=unlockedNodes();$('#map-nodes').innerHTML=WORLD.map(n=>'<button class="map-node '+(completed.includes(n.id)?'cleared ':'')+(currentNode===n.id?'current':'')+'" data-node="'+n.id+'" style="left:'+n.x+'%;top:'+n.y+'%" '+(!unlocked.has(n.id)?'disabled':'')+' aria-label="'+n.name+(completed.includes(n.id)?', cleared':unlocked.has(n.id)?', unlocked':', locked')+'"><span>'+(completed.includes(n.id)?'✓':unlocked.has(n.id)?'◆':'○')+'</span><small>'+n.name+'</small></button>').join('');$('#map-lines').innerHTML=WORLD.flatMap(n=>n.next.map(id=>{const end=WORLD.find(v=>v.id===id);return '<line x1="'+n.x*5.12+'" y1="'+n.y*1.8+'" x2="'+end.x*5.12+'" y2="'+end.y*1.8+'" stroke="'+(unlocked.has(id)?'#aaa36b':'#283421')+'" stroke-dasharray="2 4"/>'})).join('');$('#map-close').hidden=!mapReturn;renderServices()}
function travel(id){const node=WORLD.find(n=>n.id===id);if(!node||!unlockedNodes().has(id))return false;if(!node.kind&&heroes.every(h=>h.hp<=0)){tell('The party is down. Heal at town before travelling.');return false}pickedSlot=null;currentNode=id;if(node.kind){mapReturn=null;state='map';shots=[];fields=[];enemies=[];potions=[];$('#world').hidden=false;renderMap();save();build()}else{area=node.area;enter()}return true}
function completeArea(){const id='a'+area;if(!completed.includes(id))completed.push(id);mapReturn=null;state='map';openMap();tell(area===8?'Guardian defeated. All main areas cleared; you can keep exploring.':'Area cleared. New routes unlocked on the world map.');save()}
function salePrice(id){const w=ITEMS[id];return w?(w.type==='rune'?25:6+w.tier*12):0}
function renderServices(){const node=WORLD.find(n=>n.id===currentNode),service=node?.kind;$('#services').hidden=!service;$('#service-title').textContent=node?.name||'';$('#service-description').textContent=service==='town'?'Rest here to heal and revive. Select an inventory item, then sell it.':'Rare rune artifacts — 80 gold each. Select inventory items to sell.';$('#town-heal').hidden=service!=='town';const id=pickedSlot?.type==='bag'?slotItem(pickedSlot):null;$('#sell-item').disabled=!id;$('#sell-item').textContent=id?'Sell '+ITEMS[id].name+' — '+salePrice(id)+' gold':'Select inventory item to sell';$('#trader-stock').innerHTML=service==='trader'?Object.values(ITEMS).filter(w=>w.type==='rune').map(w=>'<button data-buy="'+w.id+'" '+(gold<80||!inventory.includes(null)?'disabled':'')+'>'+w.name+' · 80</button>').join(''):'';}
function healTown(){if(state!=='map'||currentNode!=='town')return false;heroes.forEach(h=>h.hp=h.maxHp);save();refresh();tell('The town healer restored and revived your party.');return true}
function sellItem(){if(state!=='map'||!['town','trader'].includes(currentNode)||pickedSlot?.type!=='bag')return false;const i=pickedSlot.index,id=inventory[i];if(!id)return false;gold+=salePrice(id);inventory[i]=null;pickedSlot=null;save();build();renderServices();tell('Sold '+ITEMS[id].name+'.');return true}
function buyRune(id){const w=ITEMS[id],slot=inventory.indexOf(null);if(state!=='map'||currentNode!=='trader'||w?.type!=='rune'||gold<80||slot<0)return false;gold-=80;inventory[slot]=id;save();build();renderServices();tell('Bought '+w.name+'.');return true}
...
function build(){
 ...
 heroes.forEach((h,i)=>stick($('#portrait-'+i).getContext('2d'),{...h,x:16,y:31,anim:0},false));restoreInfo();refresh();
}
function inspect(id){
...
function refresh(){const h=heroes[hoverHero??selected];if(!h)return;$('#class-name').textContent=classes[h.classId].name;$('#lp').textContent=Math.ceil(h.hp)+'/'+h.maxHp;$('#attack').textContent=h.atMin+'–'+h.atMax;$('#agi').textContent=h.agi.join('–');$('#range').textContent=h.range;$('#level').textContent=h.level;$('#sp').textContent=h.sp;for(const [i,name] of ['str','dex','int'].entries()){$('#'+name).textContent=h[name];const btn=$('[data-stat="'+name+'"]');btn.disabled=h.sp<1;btn.title=STAT_HELP[h.classId][i];btn.setAttribute('aria-label','Add '+name.toUpperCase()+': '+STAT_HELP[h.classId][i]);}
 ...
 $('#xp').textContent=Math.floor(h.xp)+'/'+needed(h.level);$('#xp-fill').style.width=h.xp/needed(h.level)*100+'%';$('#gold').textContent=gold;$('#area').textContent=state==='map'?'World Map':areas[Math.floor(area/3)]+': '+(area%3+1);$('#pause').textContent=paused?'Resume':'Pause';heroes.forEach((h,i)=>{const bar=$('[data-hero="'+i+'"] .life i');if(bar)bar.style.width=Math.max(0,h.hp/h.maxHp)*100+'%'});}
function update(dt){time+=dt;numbers.forEach(n=>{n.y-=13*dt;n.life-=dt});numbers=numbers.filter(n=>n.life>0);if(state!=='fight'&&state!=='walk')return;tickEffects(dt);
 for(const h of heroes){h.flash=Math.max(0,(h.flash||0)-dt);h.anim=Math.max(0,h.anim-dt);resolveStrike(h,dt);if(h.hp<=0||h===drag)continue;stepBody(h,dt);h.hp=Math.min(h.maxHp,h.hp+h.runeBonus.regen*dt);h.cooldown-=dt;h.hold=Math.max(0,h.hold-dt);h.walk=false;if(state==='walk'){h.drive=39;h.walk=true;continue}if(h.kind==='heal'&&h.cooldown<=0){const ally=heroes.filter(a=>a.hp>0&&a.hp<a.maxHp*.85).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(ally){if(Math.abs(ally.x-h.x)<h.range){shoot(h,ally,'heal',h.healing);h.cooldown=h.cool;attackMotion(h);continue}else if(h.hold<=0){h.drive=Math.sign(ally.x-h.x)*32;h.walk=true;continue}}}
 const target=enemies.filter(e=>e.hp>0).sort((a,b)=>Math.abs(a.x-h.x)-Math.abs(b.x-h.x))[0];if(!target)continue;const d=Math.abs(target.x-h.x);h.face=target.x>=h.x?1:-1;if(d>h.range&&h.hold<=0){h.drive=h.face*33;h.walk=true}if(d<=h.range+3&&Math.abs(h.y-target.y)<45&&h.cooldown<=0){h.cooldown=roll(...h.agi)/30;attackMotion(h);const token=attackToken(h),amount=basicAmount(h);if(h.kind==='melee'||h.kind==='whip')h.strike={left:.09,target,amount,token,range:h.range};else shoot(h,target,h.kind==='heal'?'magic':h.kind,amount,token)}}
 const additions=[];for(const e of enemies){if(e.hp<=0||e.frozen>0||e.stun>0)continue;e.flash=Math.max(0,e.flash-dt);e.x=Math.max(8,Math.min(505,e.x+(e.kick||0)*dt));e.kick=(e.kick||0)*Math.exp(-9*dt);e.y=floor(e.x);e.cooldown-=dt;if(e.type==='summoner'){e.summon-=dt;if(e.summon<=0&&e.remaining>0){const m=enemy('slime',Math.max(130,e.x-20));m.minion=true;m.hp=m.maxHp*=.55;additions.push(m);e.remaining--;e.summon=8;float(e.x,e.y-30,'SUMMON','#d795ee')}}const target=heroes.filter(h=>h.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;const d=Math.abs(target.x-e.x);if(d>e.range)e.x+=Math.sign(target.x-e.x)*(e.type==='boss'?13:17)*(e.slow>0?.6:1)*dt;else if(e.cooldown<=0&&Math.abs(target.y-e.y)<50){e.cooldown=e.type==='boss'?1.2:1.4;if(e.range>20)shoot(e,target,'enemy',e.at);else damage(target,e.at)}}enemies.push(...additions);
 for(const s of shots){s.life-=dt;if(s.target.hp<=0){s.life=0;continue}const dx=s.target.x-s.x,dy=s.target.y-12-s.y,d=Math.hypot(dx,dy),v=s.kind==='bullet'?330:180;if(d<v*dt+3){if(s.kind==='heal'){const n=Math.min(s.target.maxHp-s.target.hp,s.amount);s.target.hp+=n;float(s.target.x,s.target.y-22,`+${n}`,'#66ff66')}else{if(s.attack)basicHit(s.target,s.amount,s.attack);else damage(s.target,s.amount,s.element)}s.life=0}else{s.x+=dx/d*v*dt;s.y+=dy/d*v*dt}}shots=shots.filter(s=>s.life>0);enemies=enemies.filter(e=>e.hp>0);pickLoot();pickPotions();
 ...
 if(state==='walk'&&heroes.filter(h=>h.hp>0).every(h=>h.x>525)){completeArea();return}
...
function stepBody(h,dt){
 ...
 h.vx+=(h.drive-h.vx)*(1-Math.exp(-9*dt));
 let nx=h.x+h.vx*dt;if(grounded&&floor(nx)<h.y-1){h.vy=-115;nx=h.x}
 h.x=state==='walk'?nx:Math.max(8,Math.min(504,nx));h.vy+=390*dt;h.y+=h.vy*dt;
 if(h.y>=floor(h.x)){if(h.vy>80)h.leanV+=Math.sign(h.vx||1)*h.vy*.012;h.y=floor(h.x);h.vy=0}
 const targetLean=h.vx*.035+(h.vx-oldV)*.045;
 h.leanV+=((targetLean-h.lean)*95-h.leanV*16)*dt;h.lean+=h.leanV*dt;
 h.swingV+=(-h.swing*150-h.swingV*13)*dt;h.swing+=h.swingV*dt;
 h.gait+=Math.abs(h.vx)*dt*.25;h.drive=0;
}
function attackMotion(h){h.anim=.25;h.swingV=26;h.leanV+=(h.face||1)*13;h.vx=(h.vx||0)+(h.face||1)*(h.kind==='melee'?22:-9)}
function resolveStrike(h,dt){const s=h.strike;if(!s)return;s.left-=dt;if(s.left>0)return;h.strike=null;if(h.hp<=0||h===drag)return;const targets=h.kind==='whip'?enemies.filter(e=>e.hp>0&&Math.abs(e.x-h.x)<=s.range+8):[s.target];for(const e of targets)if(e.hp>0&&Math.abs(e.x-h.x)<=s.range+10&&Math.abs(e.y-h.y)<40){basicHit(e,s.amount,s.token);e.kick=(e.kick||0)+(h.face||1)*35}}
...
function stick(c,h,mark){
 ...
 const bob=Math.abs(Math.sin(g))*1.8*moving,hip=[lean*.25,-10-bob],shoulder=[lean,-18-bob];
 for(const side of [-1,1]){const phase=g+(side===1?Math.PI:0),foot=[side*4+Math.cos(phase)*5*moving-f*air*3,-Math.max(0,Math.sin(phase))*4*moving-air*3];line(c,[hip,knee(hip,foot,side),foot],col)}
 line(c,[hip,shoulder],col);c.strokeStyle=col;c.strokeRect(shoulder[0]-3,shoulder[1]-7,6,6);
 const hand=[shoulder[0]+f*(6+Math.sin(swing)*5),shoulder[1]+5-Math.sin(swing)*5];
 line(c,[shoulder,knee(shoulder,hand,-f),hand],col);line(c,[shoulder,[shoulder[0]-f*4,-13-bob],[shoulder[0]-f*5,-9-bob]],col);
 if(h.weapon){const kind=classes[h.classId].kind,weaponColor=ITEMS[h.weapon]?.color||'#bbb';if(kind==='melee'){const angle=-1.2+swing*1.7,length=h.classId===5?23:13;line(c,[hand,[hand[0]+f*Math.cos(angle)*length,hand[1]+Math.sin(angle)*length]],weaponColor)}else if(kind==='arrow'){line(c,[[hand[0],hand[1]-9],[hand[0]+f*4,hand[1]],[hand[0],hand[1]+8]],'#bd925c');line(c,[[hand[0],hand[1]-9],[hand[0]-f*swing*3,hand[1]],[hand[0],hand[1]+8]],'#aaa')}else if(kind==='whip'){const points=[hand];for(let i=1;i<6;i++)points.push([hand[0]+f*i*4,hand[1]+Math.sin(time*9-i*.7+swing)*i*.9]);line(c,points,weaponColor)}else{line(c,[[hand[0],hand[1]+8],[hand[0]+f*swing*3,hand[1]-11]],'#b7a470');c.fillStyle=weaponColor;c.fillRect(hand[0]+f*swing*3-1,hand[1]-13,3,3)}}
...
canvas.addEventListener('pointermove',e=>{if(!drag){const p=point(e),i=heroes.findIndex(h=>Math.hypot(h.x-p.x,h.y-13-p.y)<18);if(i>=0)inspectHero(i);else{restoreInfo();refresh()}return}if(paused)return;const p=point(e);drag.x=Math.max(8,Math.min(503,p.x));drag.y=Math.max(45,Math.min(floor(drag.x),p.y+13));drag.vy=0;drag.vx=0;drag.strike=null});function release(){if(drag){drag.hold=.3;drag.drive=0;drag=null}}canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);
$('#pause').onclick=toggle;$('#speed').onclick=()=>{speed=speed===1?2:1;$('#speed').textContent=speed+'x'};$('#start').onclick=start;$('#retry').onclick=()=>{currentNode='town';mapReturn=null;state='town';$('#result').hidden=true;openMap()};$('#new').onclick=()=>{if(heroes.length&&!confirm('Start a new party? This replaces your current run.'))return;state='setup';paused=false;$('#setup').hidden=false;$('#result').hidden=true;$('#world').hidden=true;tell('Choose any combination of four classes.');refresh()};$('#fullscreen').onclick=async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await $('#app').requestFullscreen()}catch{tell('Full screen is unavailable in this browser.')}};
...
$('#map-close').onclick=()=>{if(!mapReturn)return;state=mapReturn;mapReturn=null;$('#world').hidden=true;refresh()};
...
app.addEventListener('click',e=>{if(suppressGearClick)return;const slot=slotFrom(e.target);if(!slot)return;if(pickedSlot){const from=pickedSlot;pickedSlot=null;moveItem(from,slot);build()}else if(slotItem(slot)){pickedSlot=slot;inspect(slotItem(slot));tell('Item selected. Click a destination slot, or drag it there.');e.target.closest('[data-slot]').classList.add('item-picked');if(state==='map')renderServices()}});
app.addEventListener('mouseover',e=>{const slot=slotFrom(e.target),card=e.target.closest('[data-hero]');if(slot)inspect(slotItem(slot));else if(card)inspectHero(+card.dataset.hero)});app.addEventListener('focusin',e=>{const slot=slotFrom(e.target),card=e.target.closest('[data-hero]');if(slot)inspect(slotItem(slot));else if(card)inspectHero(+card.dataset.hero)});
...
function previews(){document.querySelectorAll('#choices select').forEach((el,i)=>{const c=$(`#choice-${i}`).getContext('2d');c.clearRect(0,0,32,40);stick(c,{...hero(+el.value,i),x:16,y:35},false)})}$('#choices').onchange=previews;previews();if(load()){state='town';currentNode='town';$('#setup').hidden=true;openMap();build()}else{heroes=[0,2,3,4].map(hero);build()}
