...
The journey starts in a Town stage with a shop and inn. Healing and reviving at the inn are free. Shops buy inventory items. Town shop stock follows cleared areas and stays at least one loot tier behind the latest cleared area's drop ceiling; starting stock is basic weapons. Nine combat areas each contain 5–8 stages, with one boss on the final stage. Each stage loads its fixed encounter on entry. Only purple summoners spawn extra enemies (maximum three each). Walk right after a clear to enter the next stage; clearing the final boss stage unlocks the next map locations. Undiscovered locations and paths are hidden entirely. Grassland 3 branches to the Rune Trader, who has a separate stage and shop. Rune stock expands with cleared areas, at 80 gold per rune. Cleared areas can be revisited.
